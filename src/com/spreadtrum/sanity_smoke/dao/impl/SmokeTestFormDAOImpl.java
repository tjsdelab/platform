...
public class SmokeTestFormDAOImpl implements SmokeTestFormDAO {
	...
	public List<String> searchSmokeTableNameByProject(String project) {
		String hql;
		List<String> results = null;
		hql = "select stf.testFormName from SmokeTestForm as stf where stf.projectID.projectName like '%"+project+"%' order by stf.testDate desc" ;
	    //开启session,与HttpSession完全没有任何关系，相当于一个数据库连接对象
		org.hibernate.Session session = new HibernateUtilForSS().openSession();
		Transaction tx = session.beginTransaction();
		try{
	...
	public List<String> searchSmokeTableNameByVersion(String version) {
		String hql;
		List<String> results = null;
		hql = "select stf.testFormName from SmokeTestForm as stf where stf.versionForNum like '%"+version+"%' order by stf.testDate desc" ;
	    //开启session,与HttpSession完全没有任何关系，相当于一个数据库连接对象
		org.hibernate.Session session = new HibernateUtilForSS().openSession();
		Transaction tx = session.beginTransaction();
		try{
	...
	public List<Date> searchSmokeTableNameByDate(Date sdate) {
		...
		hql = "select stf.testFormName from SmokeTestForm as stf where stf.testDate = '"+ sdate +"'" ;
	    //开启session,与HttpSession完全没有任何关系，相当于一个数据库连接对象
		org.hibernate.Session session = new HibernateUtilForSS().openSession();
		Transaction tx = session.beginTransaction();
		try{
